...
The first one is the more classic one, you request items, then you build your datacube,
and then you can mask pixels using a cloudmask asset.
...
s2_datacube = eds.legacy.datacube(
    "sentinel-2-l2a",
...
# Request items
items = eds.legacy.search(
    "sentinel-2-l2a",
    intersects=geometry,
    datetime=["2022-08-01", "2022-08-09"],
)

# Create datacube (independent from being log into earthdatastore)
# We request the "scl" assets which is the native cloudmask
s2_datacube = earthdaily.legacy.earthdatastore.datacube(
...
s2_datacube = earthdaily.legacy.earthdatastore.mask.Mask(s2_datacube, intersects=geometry).scl(mask_statistics=True)

s2_datacube = earthdaily.legacy.earthdatastore.mask.filter_clear_cover(s2_datacube, 50)  # at least 50% of clear pixels

...
class MemoryManager:
    ...
    def calculate_time_chunks(dataset: xr.Dataset, max_memory_mb: Optional[float] = None) -> int:
        ...
        logger.debug(f"Memory per date: {bytes_per_date:.2f}MB, Total: {(bytes_per_date*dataset.time.size):.2f}MB")
        logger.debug(f"Time chunks: {time_chunks} (total time steps: {dataset.time.size})")
...
class SpatialIndexer:
    ...
    @staticmethod
    def rasterize_geometries(gdf: gpd.GeoDataFrame, 
                           dataset: xr.Dataset,
                           all_touched: bool = False) -> Tuple[np.ndarray, List[np.ndarray]]:
...
class StatisticalOperations:
    ...
    def zonal_stats(dataset: xr.Dataset,
                          ...
                          reducers: List[str]) -> xr.DataArray:
    ...
    **kwargs : dict
    ...
    if method == "numpy":
        return _compute_numpy_stats(
            dataset, geometries, lazy_load, max_memory_mb,
            reducers, all_touched, preserve_columns, **kwargs
        )
    elif method == "xvec":
        return _compute_xvec_stats(
            dataset, geometries, reducers, all_touched,
            preserve_columns, **kwargs
        )
    else:
    ...
    **kwargs
    ...
    if "time" in dataset.dims and not lazy_load:
        ...
        stats = StatisticalOperations.zonal_stats(
            dataset, positions, reducers
        )
    
    # Format output
    return _format_numpy_output(
        stats, features, geometries, reducers, preserve_columns
    )
...
def _process_time_chunks(
    ...
    for time_idx in trange(0, dataset.time.size, time_chunks):
        ...
        chunk_stats = StatisticalOperations.zonal_stats(
            ds_chunk, positions, reducers
        )
        logger.debug(
...
def _format_numpy_output(
    ...
    geometry_wkt = (geometries.geometry
                   .iloc[feature_indices - 1]
...
def _preserve_geometry_columns(
    ...
    cols = [col for col in geometries.columns 
            if col != geometries._geometry_column_name]

...
def xr_wt(datacube, lmbd, time="time", weights = None, a = 0.5, min_value = -np.inf, 
          max_value = np.inf, max_iter = 10):
    ...
    if not isinstance(datacube.chunk, type(None)):
        lmbd = lmbd.chunk({k: v for k, v in datacube.unify_chunks().chunksizes.items() if k in lmbd.dims})

    # Apply whittaker smoothing along axis.
    datacube = xr.apply_ufunc(
        _wt, datacube, A, lmbd, weights, a, min_value, max_value, max_iter,
        input_core_dims = icd,

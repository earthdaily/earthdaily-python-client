...
class TestZonalStats(unittest.TestCase):
    ...
    def test_basic(self):
        zonalstats = zonal_stats(self.datacube, self.gdf, method="numpy", reducers=["min", "max"], all_touched=False)
        for operation in ["min", "max"]:
            self._check_results(zonalstats["first_var"].sel(zonal_statistics=operation).values, operation=operation)
    ...
    def test_compare_ed_xvec(self):
        ...
        for method in ["numpy", "xvec"]:
            res[method] = zonal_stats(self.datacube, self.gdf, method="numpy", reducers=["max"], all_touched=False)

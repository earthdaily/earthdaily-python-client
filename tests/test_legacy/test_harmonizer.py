...
class TestHarmonizer(unittest.TestCase):
    ...
    def test_check_timerange(self):
        ...
        valid_item_date = datetime.strptime("2023-09-12T15:10:07Z", "%Y-%m-%dT%H:%M:%SZ")

        invalid_item_date = datetime.strptime("2024-09-27T05:40:02Z", "%Y-%m-%dT%H:%M:%SZ")

        self.assertTrue(Harmonizer.check_timerange(xcal_item_from_20230901_to_20230915, valid_item_date))

        self.assertFalse(Harmonizer.check_timerange(xcal_item_from_20230901_to_20230915, invalid_item_date))

    def test_apply_to_asset_single_function(self):
        single_function_xcal = {"single_func": [{"single_func": [{"scale": 2, "offset": 0.5}]}]}
        ...
        try:
            np.testing.assert_array_equal(ds_["first_var"].loc[dict(time="1987-04-22")], expected_array)
    ...
    def test_different_xcal_by_date(self):
        ...
        # Creating fake item
        item_1987 = self.generate_fake_collection_item("item_1987", "1987-04-22T12:18:59Z")

        item_1998 = self.generate_fake_collection_item("item_1998", "1998-07-29T12:18:59Z")

        fake_items = [item_1987, item_1998]

        harmonized_fake_ds = Harmonizer.harmonize(fake_items, fake_ds, fake_xcal_item, ["first_var"])
